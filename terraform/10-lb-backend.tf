...
resource "google_compute_region_health_check" "lb-back-hc" {
  name = "lb-back-hc"
  # region = "us-central1"
...
resource "google_compute_region_backend_service" "lb-back" {
  ...
  health_checks         = [google_compute_region_health_check.lb-back-hc.self_link]

  # Named port from MIG
  port_name = "wk10-webserver"



  # Use console defaults for this
  backend {
    group           = google_compute_region_instance_group_manager.wk10mig.instance_group
    capacity_scaler = 1.0
