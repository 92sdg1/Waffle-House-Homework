...
data "google_compute_zones" "wk10-zone" {
  status = "UP"
...
resource "google_compute_region_instance_group_manager" "wk10mig" {
#  depends_on         = [google_compute_router_nat.iowa]
  ...
  distribution_policy_zones = data.google_compute_zones.wk10-zone.names
